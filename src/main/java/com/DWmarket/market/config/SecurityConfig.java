...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception{
        ...
        http.authorizeRequests()
                .mvcMatchers("/","favicon.ico", "/members/**", "/board/**", "/images/**","/image/**","members/pwchange/**","/hns/**")   //주소에 이런식으로 표현되는 경로에 대해서는 모든 유저에 대하여 권한을 주겠다.
                .permitAll().mvcMatchers("/admin/**").hasRole("ADMIN")  //주소에 이런식으로 표현되는 경로에 대해서는 관리자에 대하여만 권한을 주겠다.
                .anyRequest().authenticated();

        http.exceptionHandling().accessDeniedPage("/main.html");
